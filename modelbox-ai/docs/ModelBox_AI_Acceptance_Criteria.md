...
| G8 **NOT MET — OIDC done, SAML outstanding** | SSO via SAML 2.0 and OIDC against Entra ID, Okta and Ping; local password auth survives for air-gapped installs with no IdP to federate with | `test_federated_identity.py` (11) — an OIDC subject resolves to a local user, keyed on (issuer, subject) and never email, with just-in-time provisioning gated on an allowlist that is **empty by default**. `test_jwks_rotation.py` (8) — key rotation with real RSA signatures: an unseen `kid` triggers one cooled-down refetch, a failed fetch never discards cached keys, and a configured JWKS never falls back to the static PEM. **Why this is NOT MET despite working OIDC:** the criterion says SAML 2.0 *and* OIDC, and SAML is not implemented. The register allows no partial credit — a criterion at 90% is NOT MET, because the 10% is what a buyer's security reviewer will find, and "we support SSO" collapsing to "we support the half your IdP does not use" is precisely that finding. Either SAML lands or the criterion is deliberately narrowed to OIDC; marking it met on OIDC alone is the overclaim this register exists to stop. **Also unverified:** no test runs against a containerised Entra/Okta/Ping, so the protocol is asserted and the vendor is not | 6.5 |
...
| G10 **MET** | RBAC with viewer / modeller / approver / admin, enforced at the API rather than in the UI | Per-role authorisation tests on every mutating endpoint. The **approver** role is what makes "who signed off on this model" answerable, which is the question a remediation programme exists to answer | 6.5 |
| G11 **MET** | Audit-log export covering authentication, authorisation changes, model mutations and artifact generation, in a format shippable to Splunk or Sentinel without a custom parser | Export fixture asserted against a declared schema. Distinct from the egress ledger: that records *what left the network*, this records *who did what inside the appliance*, and a supervisor asks both | 6.5 |
